...
def run_common_commands():
    subprocess.run(['chmod', 'u=rwx,g=rwx,o=rwx', 'Health_files/', 'Health_files2/'])
    # List all .sh files
...
def run_platform_specific_commands():
    ...
    if system_name == 'darwin':
        ...
        print(f"Unrecognized system: {system_name}. This script currently supports Linux and macOS.")
...
# Define the path to the AppleScript file (saved as a plain text .scpt file)
file_path = f'{HS_path}/darna_launch.scpt'

# Open the file in write mode and write the content
with open(file_path, 'w') as file:
    file.write(applescript_content)
...
temp_script_path = f'{HS_path}/set_icon.scpt'
with open(temp_script_path, 'w') as file:
    file.write(set_icon_script)
